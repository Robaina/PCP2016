function [maprxnvalues,modelgenevalues]=mapgene2rxn(GEM,genenames,genevalues,modellogic,onlygenes)
...
%onlygenes: logical, if 1 then only genes values are retrieved, if 0, both
%genes values and the mapping to reactions are retrieved

%Semidán, October, 2014
%**************************************************************************
...
for i=1:length(modelrules),
    modelrules{i}=char(modelrules{i});
end
...
elseif onlygenes==0,    
    ...
    if numerrors>0,
       ...
       disp('');
       [num2str(numerrors),mrm{numerrors}]
    end

    maprxndata=[];
    for i=1:length(mrm),
        if isempty(mrm{i}) | isnan(mrm{i}) | ~isempty(strfind(mrm{i},'x')),
            mrm{i}='0';
        end
       maprxndata(i)=str2num(mrm{i});
    end
    maprxndata(isnan(maprxndata))=0;
    modelgenevalues=modeldata';
